...
class AppCoordinator: NSObject, Coordinator {
    ...
    // To Car Selection scene
    func toCarSelection() {
        let carSelectionScene = CarSelectionCoordinator(navigationController: navigationController)
        carSelectionScene.parentCoordinator = self
        childCoordinators.append(carSelectionScene)
        
        carSelectionScene.start()
...
extension AppCoordinator: UINavigationControllerDelegate {
    ...
    func navigationController(_ navigationController: UINavigationController, didShow viewController: UIViewController, animated: Bool) {
        ...
        // We’re still here – it means we’re popping the view controller, so we can check whether it’s a Categories view controller
        if let homeViewController = fromViewController as? FlowControlViewController {
